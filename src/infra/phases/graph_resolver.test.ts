...
describe("graph resolver", () => {
	...
	it("classifies diamonds with universal utilities correctly", async () => {
		const inventory: ProjectInventory = {
			projects: {
				app: {
					id: "app",
		...
		expect(graph.diamonds).toHaveLength(1);
		expect(graph.diamonds[0]?.pattern).toBe("universal-utility");
		expect(graph.diamonds[0]?.projectId).toBe("app");
		expect(graph.diamonds[0]?.directDependency).toBe("logger");
		expect(graph.diamonds[0]?.transitiveThrough).toContain("feature-a");
		expect(graph.diamonds[0]?.suggestion).toContain(
			"This is expected - logger is designed to be used everywhere",
		);
	});
	...
	it("detects diamond patterns as incomplete abstractions", async () => {
		const inventory: ProjectInventory = {
			projects: {
				"ui-components": {
					id: "ui-components",
		...
		expect(graph.diamonds.length).toBeGreaterThan(0);
		const diamond = graph.diamonds[0];
		expect(diamond?.pattern).toBe("incomplete-abstraction");
		expect(diamond?.suggestion).toContain("may be intentional");
	});
