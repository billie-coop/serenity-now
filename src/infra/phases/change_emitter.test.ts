...
describe("change emitter", () => {
  ...
  it("handles malformed tsconfig.json gracefully", async () => {
    const files = new InMemoryFileSystem({
      "/repo/apps/app/package.json": JSON.stringify({
        name: "@repo/app",
        dependencies: {},
      }),
      "/repo/apps/app/tsconfig.json": "{ invalid json }",
      "/repo/packages/lib/package.json": JSON.stringify({ name: "@repo/lib" }),
      "/repo/packages/lib/tsconfig.json": JSON.stringify({
    ...
    // Should handle malformed tsconfig gracefully (line 365 catch block)
    const result = await emitter.emit(
      graph,
      baseInventory,
      baseConfig,
