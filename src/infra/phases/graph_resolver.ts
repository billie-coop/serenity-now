...
function collectDependencies(record?: ProjectUsage["usage"][string]): string[] {
	if (!record) return [];
	return Array.from(
		new Set([...record.dependencies, ...record.typeOnlyDependencies]),
	);
}
	...
	function dfs(projectId: string, path: string[]): void {
		if (stack.has(projectId)) {
			...
			if (cycleStart >= 0) {
				...
				const projectsInCycle = cyclePath
	...
	function getTransitiveDependencies(
		...
		for (const [depId, _dep] of Object.entries(project.dependencies)) {
			...
			if (depProject) {
				...
				for (const [subDepId, throughProjects] of Object.entries(
					subTransitive,
				)) {
					if (!transitive[subDepId]) {
						transitive[subDepId] = new Set();
		...
		for (const directDep of directDeps) {
			if (transitiveDeps[directDep]) {
				...
				if (universalUtilities.has(directDep)) {
					...
					const isUILayer =
						projectId.includes("ui") || projectId.includes("components");
					const isDataLayer =
						directDep.includes("db") || directDep.includes("data-sync");
...
export function createGraphResolver(
	...
	return {
		async resolve(
			...
			for (const [projectId, projectInfo] of Object.entries(
				inventory.projects,
			)) {
				...
				for (const depId of collectDependencies(usageRecord)) {
					...
					// resolveWorkspaceDependencyId ensures this exists in the inventory
